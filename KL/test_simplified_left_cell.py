...
from KL.SimplifiedLeftCellModule import SimplifiedLeftCellModule
from perm import (
...
def test_simple_reflection_action(module, n, verbose=True):
    """
    Test the action of simple reflections on basis elements.

    Args:
        module: A SimplifiedLeftCellModule instance
...
def test_for_arbitrary_n(n, max_display=10, verbose=True):
    """
    Test the SimplifiedLeftCellModule with an arbitrary value of n.
    ...
    # Create module
    module = SimplifiedLeftCellModule(n)
    ...
    test_simple_reflection_action(module, n, verbose)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python test_simplified_left_cell.py <n> [--verbose] [--full-test]")
        sys.exit(1)
    ...
    # Create module
    print(f"Testing SimplifiedLeftCellModule for S_{n}")
    start_time = time.perf_counter()

    module = SimplifiedLeftCellModule(n)

    # Run tests
    action_ok = test_simple_reflection_action(module, n, verbose)
